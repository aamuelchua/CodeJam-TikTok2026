...
class RAGPipeline:
    ...
    def __init__(self, catalog_path: str | Path = "data/catalog.jsonl") -> None:
        ...
        self.faiss_index = get_vectorstore()
        self.faiss_asins = []
        asins_file = Path(__file__).resolve().parent / "faiss_index" / "asins.json"
        if asins_file.exists():
            with asins_file.open("r", encoding="utf-8") as f:
                self.faiss_asins = json.load(f)
        self._build_index()
    ...
    def execute_turn(
        ...
        if not tokens:
            ...
            bm25_asins = [str(r[0]) for r in rows]
        else:
            pos_expression = " OR ".join(f'"{t}"' for t in tokens)
            rows = self.connection.execute(
                "SELECT parent_asin FROM products WHERE products MATCH ? "
                "ORDER BY bm25(products, 0.0, 12.0, 12.0, 6.0, 2.0, 1.0, 1.0) LIMIT 30",
                (pos_expression,),
            ).fetchall()
            bm25_asins = [str(r[0]) for r in rows]

        vec_asins = []
        if self.faiss_index is not None and self.embed_model is not None and self.faiss_asins:
            try:
                q_text = all_text[:200] if all_text.strip() else msg
                q_emb = self.embed_model.encode([q_text], normalize_embeddings=True, show_progress_bar=False).astype("float32")
                _, top_indices = self.faiss_index.search(q_emb, 25)
                vec_asins = [self.faiss_asins[idx] for idx in top_indices[0] if idx < len(self.faiss_asins)]
            except Exception:
                vec_asins = []

        if vec_asins:
            rrf = {}
            for rank, asin in enumerate(bm25_asins):
                rrf[asin] = rrf.get(asin, 0.0) + (0.85 / (60.0 + rank + 1))
            for rank, asin in enumerate(vec_asins):
                rrf[asin] = rrf.get(asin, 0.0) + (0.15 / (60.0 + rank + 1))
            final_asins = sorted(rrf.keys(), key=lambda a: rrf[a], reverse=True)[:top_k]
        else:
            final_asins = bm25_asins[:top_k]

        recommendations = [{"parent_asin": a} for a in final_asins]
...
def get_vectorstore():
    ...
    if _vectorstore_singleton is None:
        try:
            import faiss
            faiss_path = Path(__file__).resolve().parent / "faiss_index" / "index.faiss"
            if faiss_path.exists():
                _vectorstore_singleton = faiss.read_index(str(faiss_path))
        except Exception:
            _vectorstore_singleton = None
